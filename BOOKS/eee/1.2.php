...
<meta charset="utf-8" />
<meta name="description" content="Начало работы с языком C#. Где скачать .NET Framework и .NET Core. Установка необходимого инструментария. Среда Visual Studio. Создание первой программы на C#.">
...
<h2>Начало работы. Visual Studio</h2><div class="date">Последнее обновление: 05.09.2019</div>
...
Visual Studio.</p>
<p>Для создания приложений на C# будем использовать бесплатную и полнофункциональную среду разработки - Visual Studio Community 2019, которую можно загрузить по следующему адресу: 
<a href="https://www.visualstudio.com/en-us/downloads" rel="nofollow">Microsoft Visual Studio 2019</a>. Также можно использовать Visual Studio 2017.</p>
<img src="./pics/1.7.png" alt="Установка Visual Studio 2019" />
...
В нашем случае нам будет интересовать прежде всего C# и .NET Core. Поэтому в наборе рабочих нагрузок можно выбрать только пункт <span class="b">Кроссплатформенная разработка .NET Core</span>. Можно выбрать и больше опций или вообще 
все опции, однако стоит учитывать свободный размер на жестком диске - ем больше опций будет выбрано, соответственно тем больше места на диске будет занято.</p>
<img src="./pics/1.8.png" alt="Установка Visual Studio 2019" />
...
<img src="./pics/1.9.png" alt="Создание первого проекта в Visual Studio 2019" />
<p>На следующем окне в качестве типа проекта выберем <span class="b">Console App (.NET Core)</span>, то есть мы будем создавать консольное приложение на языке 
C#</p>
<img src="./pics/1.10.png" alt="Проект конольного приложения на C# и .NET Core в Visual Studio 2019" />
<p>Далее в поле Project Name дадим проекту какое-либо название. В моем случае это <span class="ii">HelloApp</span>. А также укажем папку, где будет 
располагаться проект. И нажмем Create (Создать).</p>
...
структура: узел Properties или Свойств (он хранит файлы свойств приложения и пока нам не нужен); узел <span class="b">Dependencies</span> - 
это узел содержит сборки dll, которые добавлены в проект по умолчанию. Эти сборки как раз содержат классы библиотеки .NET, которые будет использовать 
C#. Однако не всегда все сборки нужны. Ненужные потом можно удалить, в то же время если понадобится добавить какую-нибудь нужную библиотеку, 
то именно в этот узел она будет добавляться.</p>
<p>Далее идет непосредственно сам файл кода программы <span class="b">Program.cs</span>. 
Как раз этот файл и открыт в центральном окне. Вначале разберем, что весь этот код представляет:</p>
<pre class="brush:c#;">
...
<p>В начале файла идет директива <span class="b">using</span>, после которой идет название подключаемого пространства имен. 
<span class="b">Пространства имен</span> представляют собой организацию классов в общие блоки. 
Например, на первой строке <code>using System;</code> подключается пространство имен System, которое содержит фундаментальные и базовые классы платформы .NET.</p>
...
сначала идет ключевое слово <span class="b">namespace</span>, после которого название пространства имен. По умолчанию Visual Studio дает ему название 
проекта. Далее внутри фигурных скобок идет блок пространства имен.</p>
<p>Пространство имен может включать другие пространства или классы. В данном случае у нас по умолчанию сгенерирован один класс - Program. Классы 
объявляются похожим способом - сначала идет ключевое слово <span class="b">class</span>, а потом название класса, и далее блок самого класса в фигурных скобках.</p>
<p>Класс может содержать различные переменные, методы, свойства, прочие инструкции. В данном случае у нас объявлен один метод Main. В программе на C# метод Main является входной точкой программы, с него начинается все управление. Он обязательно 
должен присутствовать в программе.</p>
<p>Слово <code>static</code> указывает, что метод Main - статический, а слово <code>void</code> - что он не возвращает никакого значения. Позже 
...
<p>Внутри метода располагаются действия, которые этот метод выполняет. По умолчанию он содержит одно действие: <code>Console.WriteLine("Hello World!");</code> 
- оно выводит на консоль строку "Hello World!".</p>
...
то при запуске приложения мы сможем ввести свое имя, и затем оно будет выведено на консоль.</p>
<img src="./pics/1.11.png" alt="Первое приложение на C# и .NET Core" />
...
<p>По сравнению с автоматически сгенерированным кодом я внес несколько изменений. Теперь в методе Main первой строкой выводится приглашение к вводу.</p>
<pre class="brush:c#;">Console.Write("Введите свое имя: ");</pre>
<p>Класс Console, метод которого вызывается, находится в пространстве имен System. Это пространство подключено в начале 
с помощью директивы using. Без подключения пространства имен System я бы не смог использовать класс Console и вывести строку на консоль. 
Однако в принципе нам необязательно подключать пространство имен. Мы можем даже удалить первую строку, но в этом случае 
мы тогда должны будем указать полный путь до используемого класса. Например, в нашем случае мы могли бы написать: <code>System.Console.WriteLine("Введите свое имя:")</code>.</p>
...
<p>То есть с помощью метода <span class="b">Console.ReadLine()</span> мы можем считать с консоли строку.</p>
...
<p>Итак, мы создали первое приложение. Вы его можете найти на жестком диске в папке проекта в каталоге <span class="b">bin/Debug</span>. Оно будет называться по имени проекта и иметь расширение exe (для более ранних версий .NET Core это файл с расширением 
dll). И затем этот файл можно будет запускать без Visual Studio, а также переносить его на другие компьютеры, где есть .NET Core.</p>
...
<ul>
<li><a title="Видео по статье" rel="nofollow" href="https://youtu.be/y1OXLBDZT7k"><i class="fa fa-lg fa-youtube-play"></i><span class="dop-txt yt">Модуль 1. Первая программа на C# в Visual Studio 2017</span></a></li>
</ul>
</fieldset></div>
...
<div class="menT" id="menT"><ul id="browser" class="filetree"> 
	<li class="closed"><span class="folder">Глава 1. Введение в C#</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/1.4.php">Установка версии языка</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 2. Основы программирования на C#</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/2.25.php">Переменные</a></span></li>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/2.5.php">Условные конструкции</a></span></li>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/2.7.php">Программа сортировки массива</a></span></li>
	...
	<li class="closed"><span class="folder">Глава 3. Классы. Объектно-ориентированное программирование.</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.25.php">Пространства имен, псевдонимы и статический импорт</a></span></li>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.3.php">Константы, поля и структуры для чтения</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.36.php">Перегрузка операторов</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.26.php">Значение null</a></span></li>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.41.php">Сокрытие методов</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.42.php">Различие переопределения и сокрытия методов</a></span></li>
	...
	<li class="closed"><span class="folder">Глава 5. Делегаты, события и лямбды</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 7. Дополнительные возможности ООП в C#</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.34.php">Pattern matching</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.35.php">Деконструкторы</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.45.php">Паттерны switch</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/2.17.php">Nullable-типы</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/2.21.php">Переменные-ссылки и возвращение ссылки</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 8. Объектно-ориентированное программирование. Практика</span>
		<ul>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.29.php">Создание проекта библиотеки классов</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.30.php">Создание классов. Часть 1</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.31.php">Создание классов. Часть 2</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/3.32.php">Создание главного проекта</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 9. Коллекции</span>
		<ul>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/4.3.php">ArrayList</a></span></li>
	...
	<li class="closed"><span class="folder">Глава 10. Работа со строками</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 11. Работа с датами и временем</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 12. Дополнительные классы и структуры .NET</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 13. Многопоточность</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/11.9.php">Таймеры</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 14. Параллельное программирование и библиотека TPL</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 15. Aсинхронное программирование</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/13.5.php">Последовательный и параллельный вызов асинхронных операций</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/13.4.php">Обработка ошибок в асинхронных методах</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/13.6.php">Отмена асинхронных операций</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/13.8.php">Асинхронные стримы</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 16. LINQ</span>
		<ul>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.1.php">Основы LINQ</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.2.php">Фильтрация выборки и проекция</a></span></li>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.11.php">Методы Skip и Take</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.6.php">Группировка</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.7.php">Соединение коллекций. Метод Join, GroupJoin и Zip</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.10.php">Методы All и Any</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.8.php">Отложенное и немедленное выполнение LINQ</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/15.9.php">Делегаты и анонимные методы в запросах LINQ</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 17. Parallel LINQ</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 18. Рефлексия</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 19. Dynamic Language Runtime</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 20. Сборка мусора, управление памятью и указатели</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 21. Работа с потоками и файловой системой</span>
		<ul>
			...
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/6.2.php">Бинарная сериализация. BinaryFormatter</a></span></li>
			<li><span class="file"><a href="//metanit.com/sharp/tutorial/5.7.php">Архивация и сжатие файлов</a></span></li>
		</ul>
	</li>
	<li class="closed"><span class="folder">Глава 22. Работа с JSON</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 24. Процессы и домены приложения</span>
		<ul>
	...
	<li class="closed"><span class="folder">Глава 25. Валидация модели</span>
		<ul>
